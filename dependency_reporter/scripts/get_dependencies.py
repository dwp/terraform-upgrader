...
class RepoDependencies:
    ...
    def add_repo(
        self, tfstate_repo_name, repo_base_url, org, file_extension, private=False
    ):
        repo_name = self._find_and_clone_repo(
            tfstate_repo_name, repo_base_url, org, file_extension, private
        )
    ...
    def _find_and_clone_repo(
        ...
        if os.system(clone) > 0:
            req_str = (
                f"https://{repo_base_url}/api/v3"
                if private
                else f"https://api.github.com"
            )
            auth = (username, pat) if private else ""
            repo_data = requests.get(
                f"{req_str}/orgs/{org}/repos?type=all&per_page=500", auth=auth
            ).json()
            possible_matches = [
                repo["name"] for repo in repo_data if tfstate_repo_name in repo["name"]
            ]
            for repo in possible_matches:
                ...
                for file_path in result:
                    config_object = self._get_remote_state_block(
                        f"{repo}/{file_path}", 'backend "s3" {', 7
                    )[0]
                    s3_file_name = (
                        [s for s in config_object if ".tfstate" in s][0]
                        .split("/")[-1]
                        .split(".")[0]
                    )
            ...
            raise ValueError(
                f"No repo match found for {tfstate_repo_name} in {org} remote repository."
            )
        else:
