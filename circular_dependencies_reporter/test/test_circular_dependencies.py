...
class TestCircularDependencies(TestCase):
    ...
    def test_terraform_config_files(self):
        files = ['./.DS_Store', './requirements.txt', './Dockerfile', './__init__.py', './README.md', './setup.py',
                 './.env', './app.py', './docker-compose.yml', './tests/test_circular_dependencies.py',
                 './tests/__init__.py', './example/terraform.tf', './finder/get_dependencies.py',
                 './finder/__init__.py', './finder/find_circular_dependencies.py']
        with patch.object(RepoDependencies, '_get_config_files', return_value=files) as mock_method:
    ...
    def test_get_remote_state_block(self):
        block = RepoDependencies._get_remote_state_block("test/resources/terraform_example.tf",
                                                         'data "terraform_remote_state"', 10)
        with open("test/resources/terraform_state_block_example.tf", "r") as terraform_state_block_example_file:
